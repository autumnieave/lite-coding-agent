...
| 类别 | 次数 | 任务成功率 | 工具选择 | 平均步数 |
|---|---:|---:|---:|---:|
| retrieval | 12 | 91.7% | 66.7% | 2.2 |
| edit_exec | 12 | 66.7% | 33.3% | 6.3 |
| long_context | 12 | 100.0% | 100.0% | 12.3 |

36 次里达标 31 次（86.1%）。**5 次失败全部只挂在步数预算上，内容判定 36/36 全为真**——文件找对了、
改动对了、JSON 合法，只是绕路试探 shell 超过了步数上限。逐任务稳定性表与两个已修的判定缺陷见
`docs/evidence.md` 用例九。

C 类另补了 `off` 档对照（关闭约束存储、压缩照常发生，每组 12 次）：**开启机制 12/12 通过、违反 0 次；
关闭机制 6/12、违反 9 次**，其中 3 次是约束被压缩整段丢掉。两组 Tier 4 触发次数（36）与摘要消息数（1）
完全相同，差异只来自约束保护——机制买到的是「整段丢失也丢不掉」的确定性。
