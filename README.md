...
同一批实验还量化了四层压缩的两处技术债：Tier 4 触发次数从均值 12~13 / 最大 24 降到均值 5.4 / 最大 6；摘要消息数从「等于压缩次数」降到恒为 1。

> Day 4 首跑的对照组（372/400）**被高估了**：那次的数据里还残留一条隐式保留通道（旧摘要原文被反复复制进新摘要），拆掉后对照组的真实水平是 69% 左右。原因见 `docs/evidence.md` 用例五。
