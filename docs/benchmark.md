...
1. **A/B 类的 JSON 解析放宽**：回复里夹带说明文字或一层围栏都能取出 JSON（`parse_json_lenient`）。
   实现是「先找围栏、再找首尾括号」逐个试，不要求整段只由围栏构成——首轮全量跑发现按全文
   `fullmatch` 判定，会把「内容正确 + 多一句收尾话」误判成不合规（A2、B4 都踩到）。
...
6. **提示词里的示例值不得等于真值**：B2 的 `{"file_count": 12}`、B4 的 `{"total_lines": 2400}`
   在 standard 档恰好就是答案，照抄示例即可通过。改用固定占位（`EXAMPLE_COUNT` /
   `EXAMPLE_LINES`），`--self-test` 加断言防回归。
7. **汇总增加「按任务」表**：给出每条任务的成功率与稳定性（稳定通过 / 稳定失败 / 不稳定）。
   同一配置、同一 seed 重跑会出现 4/12 条翻转而聚合恰好抵消，单次运行的数字不足以支撑结论，
   定稿数据按 `--runs 3` 出。
8. **汇总表的口径**：A/B 类只有 `on` 档，C 类含 `on` 与 `off` 两组，所以「按 category」「按任务」里
   C 行的次数是两组合计（每个 C 任务 6 次），分组结论只看「按 group」表；汇总开头已写明这一点。
9. **正确性与效率拆成两列**：`内容正确` = 除 `within_steps` 外的判定是否全真，`步数达标` 只报
   `within_steps`（没有步数上限的任务显示 `-`）。`task_success = 两者都真`，混在一格会把
   「做对了但绕路」记成「做错」。`--self-test` 覆盖了「内容对但超步数」与「无步数上限」两种行。
