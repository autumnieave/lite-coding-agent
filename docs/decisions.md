...
## ADR 索引（ADR-001 ~ ADR-018）

| 编号 | 标题 | 类型 |
|---|---|---|
| ADR-001 | 不依赖 LangChain，自研 Agent Loop | 工程选型 |
| ADR-002 | 压缩阈值选在上下文窗口的 60% | 参数取舍 |
| ADR-003 | 项目记忆用 Markdown，不引入向量数据库 | 工程选型 |
| ADR-004 | 关键约束独立存储，不参与压缩 | **纯自研** |
| ADR-005 | 实现语言选 Python 而不是 TypeScript | 工程选型 |
| ADR-006 | Agent Loop 的关键取舍 | 有对照 |
| ADR-007 | 流式输出默认开启，并接受 httpcore2 关闭流时的已知噪音 | 有对照 |
| ADR-008 | `edit_file` 用「唯一性校验 + read-before-edit + mtime 防护」三重约束 | 有对照 |
| ADR-009 | `bash` 超时改为终止整棵进程树 | 有对照 |
| ADR-010 | 上下文压缩的阈值、估算方式与可观测指标 | 有对照 |
| ADR-011 | `grep` 不调用系统 grep，改用纯 Python 遍历 | 有对照 |
| ADR-012 | 约束保留机制（压缩前注入 + 压缩后校验 + 自愈） | **纯自研** |
| ADR-013 | 流式输出只做「增量打印」，不做流式工具执行 | 有对照 |
| ADR-014 | 项目记忆读 `AGENTS.md`，不做跨会话记忆库 | 部分对照 |
| ADR-015 | 会话 checkpoint 用 JSONL 追加写，恢复时裁掉未配对的尾部 | 有对照 |
| ADR-016 | 约束同时注入 system prompt，短任务也能看到 | **纯自研** |
| ADR-017 | MCP 客户端手写 JSON-RPC over stdio | 有对照 |
| ADR-018 | MCP 工具调用前过一道约束校验 | **纯自研** |

「类型」列的三种取值：**有对照**＝Claude Code 与参考项目有相邻设计，差异记在正文的「三层对照」里；
**部分对照**＝只是概念上相邻，不是同一套机制；**纯自研**＝两侧都没有，差异表里找不到对照对象；
**工程选型 / 参数取舍**＝常规工程决策，不做三层对照。
...
- 只覆盖 MCP 工具，本地工具不受影响。
- 只看工具名，不看**参数**：约束「禁止把 API Key 写进文件」拦不住「用 echo 工具把 Key 回显出来」。要做参数级校验，得先有可判定的参数规则，这条和 `docs/evidence.md` 的「行为探针覆盖面」是同一类后续迭代项。
