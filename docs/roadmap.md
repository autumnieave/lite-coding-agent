...
| `docs/decisions.md` 补 ADR | 1.5h | 至少 5 条决策记录 | ✅ 已 15 条（ADR-001 ~ ADR-015） |
...
- [x] 全量 **444** 个单测通过（Day 5 收尾时 417），`ruff check` 与 `ruff format --check` 干净
...
- [x] **M6**：前 5 天验收全打勾，评测规模扩大（Day 6 结束，444 个单测通过；**原计划的「20 用例」实际做成了「同档位 20+20 次复跑」**——提高的是样本量不是用例多样性，行为探针仍只有 3 项，这条欠债顺延 Day 7，见 Day 6 卡点）
