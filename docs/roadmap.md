...
**目标**：扩展项，二选一。**最终选 MCP**——它有公开协议可对照（参考项目第 12 章），演示成本也比子 Agent 低；子 Agent 隔离顺延。
...
**子 Agent 为什么不做**：**不是时间不够，是评估后判定没有增量**——它与「智能问数」项目在能力上重复，再花 4-5 小时做一遍「独立上下文 + 工具白名单」，能力上没有增量。MCP 反而有公开协议可对照、演示成本低，所以二选一选 MCP。
...
| Day 4 实验设计超时 | 核心机制无数据 | 未超时；但中途撞上 API 余额中断，补了 `--resume` 从已有记录续跑 |
| Day 5 时间不够 | 交付不达标 | 测试 > 文档 > GIF，遗留项顺延到第 6 天补 |
| Day 6 遗留项太多 | 文档不完整 | 优先约束实验和 decisions.md，MCP 评估后不做 |
| Day 7 时间不够 | 少扩展项 | 不做，核心链路已完整 |
...
- [x] **M5**：核心链路完成，GitHub 完整（Day 5 结束，417 个单测通过；四层压缩两处技术债已修，`memory/agents_md.py` 与 `memory/session.py` 落地但尚未接入主流程，见 `docs/evidence.md` 用例六）
- [x] **M6**：前 5 天验收全打勾，评测规模扩大（Day 6 结束，451 个单测通过；**原计划的「20 用例」实际做成了「同档位 20+20 次复跑」**——提高的是样本量不是用例多样性，行为探针仍只有 3 项，这条后续迭代项顺延 Day 7，见 Day 6 卡点）
...
> 这是分阶段交付：核心链路与约束保留机制已验证并附实测数据；后续迭代会扩大评测规模、评估子 Agent 隔离。
